...
def sync_league_results(league_id: int, season: int | None = None) -> int:
    """`season`: ano de início da temporada (ex.: 2025 para a temporada 2025-26 europeia).
    Sem isso, a API devolve só a temporada atual. Usado pra popular o cold-start de liga
    que a temporada mal começou — o modelo (`poisson_goals.py`) já usa TODO o histórico de
    `fd_matches` sem filtrar por temporada, então isto é só mais dado real na mesma
    tabela, sem tocar em nenhuma fórmula."""
    ...
    try:
        with conn.cursor() as cur:
            ...
            payload = football_data.get(f"competitions/{code}/matches", params)  # sem filtro de status: finalizados + agendados de uma vez
