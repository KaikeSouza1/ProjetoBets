...
def _fetch_matches_from_fd_matches(code: str) -> list[tuple]:
    """Fonte principal — temporada inteira via football-data.org, pra qualquer liga
    que tenha `football_data_code` (as 6 ligas semeadas em `reference.TARGET_LEAGUES`)."""
...
def _fetch_matches_from_fixtures(league_id: int) -> list[tuple]:
    """Fallback pra liga sem `football_data_code` (ex.: Copa do Brasil — football-data.org
    não cobre essa competição em nenhum plano gratuito). Usa nossa própria tabela
    `fixtures`, alimentada dia a dia pelo `fixtures_daily` (API-Football) — cresce
    devagar a partir de agora, sem bulk histórico de temporada disponível pra copas no
    plano gratuito da API-Football (testado: bloqueia a temporada atual). Mesmo formato
    de tupla que `_fetch_matches_from_fd_matches`, então `compute_strengths_from_matches`
...
def build_league_model(league_id: int) -> LeagueModel:
    ...
    if not matches:
        ...
        raise ValueError(f"liga {league_id}: nenhum resultado disponível — ver {source_hint}")
