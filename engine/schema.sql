...
CREATE TABLE IF NOT EXISTS value_bets (
    ...
    rank_score            NUMERIC(7,5) NOT NULL,
    created_at            TIMESTAMPTZ NOT NULL DEFAULT now()
);
CREATE INDEX IF NOT EXISTS idx_value_bets_rank ON value_bets (rank_score DESC);
...
CREATE TABLE IF NOT EXISTS backtest_bets (
    ...
    odd_used               NUMERIC(7,3),   -- NULL quando não há odd histórica real (ver nota no relatório)
    result                 TEXT,   -- 'ganhou' | 'perdeu' | 'anulada' — só preenchido quando odd_used existir
